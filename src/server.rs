...
impl Server {
    ...
    pub fn init(supported_transports: Vec<String>) -> Result<Option<Server>, InitError> {
        ...
        match env::var("TOR_PT_SERVER_TRANSPORTS") {
            ...
            Err(e) => {
                ...
                match e {
                    env::VarError::NotPresent => return Err(InitError::MissingEnvVarError("TOR_PT_SERVER_TRANSPORTS".to_string())),
                    env::VarError::NotUnicode(_) => return Err(InitError::ParserError(
                        format!("Could not read list of transports requested by parent process: {}", e)
                    )),
                }
            }
        }
        ...
        match env::var("TOR_PT_SERVER_BINDADDR") {
            ...
            Err(error) => {
                match error {
                    env::VarError::NotPresent => (), // This var is optional
                    env::VarError::NotUnicode(_) => return Err(InitError::ParserError(
                        format!("Could not read transport options specified by parent process: {}", error)
                    )),
                }
            },
        }
            ...
            Ok(val) => {
                    if val != "" {
                        ...
                        if !have_destination {
                            ...
                            return Err(InitError::EmptyEnvVarError("TOR_PT_EXTENDED_SERVER_PORT".to_string()));
                        }
                    }
            },
            Err(error) => match error {
                env::VarError::NotPresent => {
                    if !have_destination {
                        println!("ENV-ERROR neither TOR_PT_ORPORT nor TOR_PT_EXTENDED_SERVER_PORT set in server mode");
                        return Err(InitError::MissingEnvVarError("TOR_PT_EXTENDED_SERVER_PORT".to_string()));
                    }
                },
                env::VarError::NotUnicode(_) => return Err(InitError::ParserError(format!("Could not read extended ORPort transport options specified by parent process: {}", error))),
            },
        }
            ...
            match env::var("TOR_PT_AUTH_COOKIE_FILE") {
                Ok(val) => {
                    ...
                    match fs::File::open(val) {
                        ...
                        Err(error1) => {
                            ...
                            return Err(InitError::ReadCookieError(error1));
                        }
                    }
                    ...
                    match f.read_to_end(&mut content) {
                        ...
                        Err(error2) => {
                            ...
                            return Err(InitError::ReadCookieError(error2));
                        }
                    }
                ...
                Err(error) => match error {
                    env::VarError::NotPresent => {
                        ...
                        return Err(InitError::MissingEnvVarError("TOR_PT_AUTH_COOKIE_FILE".to_string()));
                    }
                    env::VarError::NotUnicode(_) => return Err(InitError::ParserError(
                        format!("Could not read path to extended ORPort auth cookie: {}", error)
                    )),
                },
            }
        }
